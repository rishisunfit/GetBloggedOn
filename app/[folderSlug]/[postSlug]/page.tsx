...
export default function CanonicalPostPage() {
    ...
    const headingFontOption = fontOptions.find(f => f.name === styles.headingFont) || fontOptions[0];
    const bodyFontOption = fontOptions.find(f => f.name === styles.bodyFont) || fontOptions[1];

    return (
        <div className="min-h-screen" style={{ backgroundColor: styles.backgroundColor }}>
            <HeatmapTracker postId={post.id} />
            <div className="max-w-3xl mx-auto px-4 py-12">
                {/* Template Header */}
                {template && (
                    <div className="mb-12" style={{ textAlign: template.alignment || "left" }}>
                ...
                {post.component_order && post.component_order.length > 0 ? (
                    post.component_order.map((componentType: string) => {
                        if (componentType === "quiz" && post.quiz_id && post.quiz_id !== null) {
                    ...
                    <>
                        {post.quiz_id && post.quiz_id !== null && <QuizRenderer quizId={post.quiz_id} />}
