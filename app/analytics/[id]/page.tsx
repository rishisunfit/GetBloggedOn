...
import { ArrowLeft, BarChart3, MousePointerClick, Eye, Target, AlertTriangle, Calendar } from "lucide-react";
...
export default function PostAnalyticsPage() {
    ...
    const loadData = async () => {
        try {
            ...
            const { body, template: extractedTemplate } = splitTemplateFromHtml(postData.content || "", postData.created_at);
            setPost({
                id: postData.id,
                ...
                <div className="max-w-7xl mx-auto px-4 py-8">
                                ...
                                {post && bodyHtml && safeTemplate && (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {clickData
                                                .filter(click => click.content_x !== null && click.content_y !== null)
                                                .map((click, index) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={index}
                                                            className={`absolute w-4 h-4 rounded-full border-2 border-white shadow-lg pointer-events-none z-50 ${click.is_dead_click ? "bg-red-500" : "bg-green-500"
                                                                }`}
                                                            style={{
                                                                left: `${x}px`,
                                                                top: `${y}px`,
                                                            ...
                                                            title={`${click.is_dead_click ? "Dead click" : "Click"} at (${x}, ${y})`}
                                                        />
                                                    );
                                                })}
                                        </article>
                                        ...
                                        <div key={item.section_id} className="space-y-2">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <div className="flex gap-4 text-xs text-gray-600">
                                                    ...
                                                    <span>{Math.round(item.avg_time_visible_ms / 1000)}s avg</span>
                                                </div>
                                            </div>
                                            <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${Math.min((item.avg_time_visible_ms / 30000) * 100, 100)}%`,
                                                    }}
                                                />
                                            </div>
                                        </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-xl font-bold text-gray-900">CTA Interactions</h2>
                        </div>
                        {ctaData.length > 0 ? (
                            <div className="space-y-4">
                                {ctaData.map((item) => (
                                    <div key={item.cta_id} className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <div className="flex gap-4 text-xs text-gray-600">
                                                ...
                                                <span className="font-semibold">{item.conversion_rate.toFixed(1)}% conversion</span>
                                            </div>
                                        </div>
                                        <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                                            <div
                            ...
                            <p className="text-gray-500 text-center py-8">No CTA data available yet</p>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                        <div className="flex items-center gap-3 mb-6">
                            <AlertTriangle className="text-red-600" size={24} />
                            <h2 className="text-xl font-bold text-gray-900">Rage Click Indicators</h2>
                        </div>
                        {rageClickData.length > 0 ? (
                            <div className="space-y-2">
                                ...
                                <div className="relative bg-gray-100 rounded-lg p-4" style={{ aspectRatio: "16/9", minHeight: "400px" }}>
                                    {rageClickData.map((rage, index) => {
                                        const intensity = Math.min((rage.click_count / 10) * 100, 100);
                                        return (
                                            <div
                                                key={index}
                                                ...
                                                title={`${rage.click_count} rage clicks at ${Math.round(rage.x_percent)}%, ${Math.round(rage.y_percent)}%`}
                                            />
                                        );
                                    })}
                                </div>
