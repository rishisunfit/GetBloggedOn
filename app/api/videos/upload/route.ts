...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!file || !(file instanceof File)) {
            return NextResponse.json(
                { error: "No video file provided", details: "Attach a File named 'file'" },
                { status: 400 },
            );
        }
        ...
        if (!file.type.startsWith("video/")) {
            return NextResponse.json(
                { error: "Invalid file type", details: "Only video files are supported" },
                { status: 400 },
            );
        }
            ...
            return NextResponse.json(
                {
                    ...
                    details: `Video size must be less than ${Math.floor(MAX_VIDEO_SIZE_BYTES / 1024 / 1024)}MB`,
                },
                { status: 400 },
            );
        }
        ...
        const cloudflareCustomerCode = process.env.NEXT_PUBLIC_CLOUDFLARE_STREAM_CUSTOMER_CODE;

        if (!cloudflareAccountId || !cloudflareApiToken) {
            return NextResponse.json(
                { error: "Server misconfiguration", details: "Missing Cloudflare credentials" },
                { status: 500 },
            );
        }
        ...
        if (!cloudflareCustomerCode) {
            return NextResponse.json(
                { error: "Server misconfiguration", details: "Missing CLOUDFLARE_STREAM_CUSTOMER_CODE" },
                { status: 500 },
            );
        }
            ...
            return NextResponse.json(
                {
                    ...
                    details: uploadResult.errors ?? [{ message: "Unknown error", code: -1 }],
                },
                { status: 502 },
            );
        }
