    ...
    try {
        ...
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            return NextResponse.json(
                {
                    error: "Unauthorized",
                    details: "Missing or invalid authorization token. Please log in to send a reply.",
                },
                { status: 401 }
            );
        }
        ...
        if (submission.post_author_id !== user.id) {
            return NextResponse.json(
                { error: "Unauthorized - You can only reply to submissions for your own posts" },
                { status: 403 }
            );
        }
        ...
        if (email?.trim()) {
            ...
            const emailSent = await sendTwilioEmail(email.trim(), emailSubject, emailBody);
