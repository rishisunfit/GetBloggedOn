...
export const heatmapsApi = {
    ...
    async getClickData(postId: string, limit = 1000): Promise<ClickHeatmapData[]> {
        const { data, error } = await supabase
            .from("post_heatmap_clicks")
            .select("x_percent, y_percent, content_x, content_y, scroll_y, content_container_selector, is_dead_click")
            .eq("post_id", postId)
            .limit(limit);
    ...
    async getRageClickData(postId: string): Promise<RageClickHeatmapData[]> {
        ...
        data?.forEach((item) => {
            const key = `${Math.floor(item.x_percent / 5)}-${Math.floor(item.y_percent / 5)}`;
            if (!locations[key]) {
                locations[key] = {
