...
export function splitTemplateFromHtml(html: string, createdDate?: string): { template: PostTemplateData; body: string } {
    ...
    const template = normalizeTemplateData(hasAny ? extracted : null, createdDate);
...
export function hasTemplateStructure(content: string | any): boolean {
    ...
    if (typeof content === "object" && content !== null) {
        if (content.type === "doc" && Array.isArray(content.content)) {
            ...
            return ["templateSeries", "templateTitle", "templateSubtitle", "templateByline"].every(
                type => nodeTypes.includes(type)
            );
        }
    }
 ...
 */
export function ensureTemplateStructure(content: string | any, createdDate?: string): string | any {
