...
export function HeatmapVisualization({ postId, type }: HeatmapVisualizationProps) {
    ...
    return (
        <div className="p-4 border rounded-lg">
            {type === "scroll" && <ScrollVisualization data={data as ScrollHeatmapData[]} />}
            {type === "clicks" && <ClickVisualization data={data as ClickHeatmapData[]} />}
            {type === "attention" && <AttentionVisualization data={data as AttentionHeatmapData[]} />}
            {type === "cta" && <CTAVisualization data={data as CTAHeatmapData[]} />}
            {type === "rage-clicks" && <RageClickVisualization data={data as RageClickHeatmapData[]} />}
        </div>
    );
}
                ...
                {data.map((item) => {
                    ...
                    return (
                        <div key={item.scroll_bucket} className="flex items-center gap-4">
                            <div className="w-20 text-sm font-medium">{item.scroll_bucket}%</div>
...
function ClickVisualization({ data }: { data: ClickHeatmapData[] }) {
    ...
    return (
        <div>
            ...
            <div className="relative border-2 border-gray-300 rounded-lg" style={{ aspectRatio: "16/9", minHeight: "300px" }}>
                {data.slice(0, 500).map((item, index) => (
                    <div
                        key={index}
                        className={`absolute w-3 h-3 rounded-full ${item.is_dead_click ? "bg-red-500 opacity-60" : "bg-blue-500 opacity-40"
                            }`}
                        style={{
                            left: `${item.x_percent}%`,
                            top: `${item.y_percent}%`,
                        ...
                        return (
                            <div key={item.section_id}>
                                <div className="flex items-center justify-between mb-1">
                                    <div className="text-sm font-medium truncate">{item.section_id}</div>
                ...
                {data.map((item, index) => {
                    ...
                    return (
                        <div
                            ...
                            title={`${item.count} rage click events, ${item.totalClicks || item.click_count} total clicks`}
                        />
                    );
                })}
            </div>
