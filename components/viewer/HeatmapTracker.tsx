...
export function HeatmapTracker({ postId, enabled = true }: HeatmapTrackerProps) {
    ...
    const ctaDataRef = useRef<Map<string, CTAData>>(new Map());
    const rageClickDataRef = useRef<Map<string, { clicks: number; firstClick: number }>>(new Map());
    const sectionObserversRef = useRef<Map<string, IntersectionObserver>>(new Map());
    ...
    const trackClick = useCallback(
        (event: MouseEvent) => {
            ...
            const xPercent = Number(((event.clientX / documentWidth) * 100).toFixed(2));
            const yPercent = Number(((event.clientY / documentHeight) * 100).toFixed(2));

            // Find the content container (article or .preview-content)
            const contentContainer = target.closest("article") ||
            ...
            const elementClass = target.className && typeof target.className === "string" ? target.className : null;
            ...
            if (existing) {
                ...
                if (timeSinceFirst < 500) {
                    ...
                    rageClickDataRef.current.set(key, { clicks: newCount, firstClick: existing.firstClick });
