...
const SettingsIcon = `<svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>`;

export function VideoTimestamps({ postId, containerSelector = ".preview-content" }: VideoTimestampsProps) {
            ...
            const initPlayer = async () => {
                try {
                    ...
                    try {
                        ...
                        timestamps.sort((a, b) => a.timestamp_seconds - b.timestamp_seconds);
                    } catch (e) {
    ...
    const createOverlayPlayer = (
        ...
        const wrapper = iframe.closest(".video-wrapper") as HTMLElement || iframe.parentElement as HTMLElement;
        if (!wrapper) return;
        ...
        `;
        centerPlay.querySelector("svg")!.style.cssText = "width: 40px; height: 40px; margin-left: 4px;";
        centerPlay.onmouseenter = () => { centerPlay.style.transform = "translate(-50%, -50%) scale(1.1)"; centerPlay.style.background = primaryColor; };
        centerPlay.onmouseleave = () => { centerPlay.style.transform = "translate(-50%, -50%) scale(1)"; centerPlay.style.background = "rgba(0,0,0,0.6)"; };
        ...
        const markerElements: Array<{ ts: VideoTimestamp; dot: HTMLElement; label: HTMLElement }> = [];
        timestamps.forEach((ts) => {
            ...
            label.innerHTML = `
                <div style="font-weight: 600; margin-bottom: 2px; font-size: 7px;">${ts.label}</div>
                <div style="font-size: 6px; opacity: 0.9; font-family: tabular-nums;">${formatTime(ts.timestamp_seconds)}</div>
            `;
            label.style.cssText = `
                position: absolute;
        ...
        speedWrapper.style.cssText = "display: flex; align-items: center; gap: 6px;";
        ...
        container.onmouseenter = () => { isHovering = true; showControls(); };
        container.onmouseleave = () => { isHovering = false; resetHideTimer(); };
        container.onmousemove = () => resetHideTimer();
        container.onclick = (e) => {
            const target = e.target as HTMLElement;
            // Don't trigger if clicking on any interactive element
            if (target.closest("button, input, select, .cf-seek-track, .cf-timestamp-dot, .cf-controls-bar, .cf-center-play")) {
                return;
            }
                ...
                try {
                    ...
                    if (typeof player.paused === "function") {
                        ...
                        try {
                            ...
                            paused = typeof result === "boolean" ? result : (result as any)?.then ? true : player.paused ?? true;
                        } catch {
                            paused = player.paused ?? true;
                        }
                    } else if (typeof player.paused === "boolean") {
                    ...
                    if (centerSvg) {
                        centerSvg.style.cssText = "width: 40px; height: 40px; margin-left: " + (isPlaying ? "0" : "4px") + ";";
                    }
