...
import { X, BarChart3, MousePointerClick, Eye, Target, AlertTriangle } from "lucide-react";
import { heatmapsApi, type ScrollHeatmapData, type ClickHeatmapData, type AttentionHeatmapData, type CTAHeatmapData, type RageClickHeatmapData } from "@/services/heatmaps";
import { postsApi, PostStyles } from "@/services/posts";
import { splitTemplateFromHtml, normalizeTemplateData, type PostTemplateData } from "@/services/postTemplate";
...
export function AnalyticsDrawer({ isOpen, onClose, postId }: AnalyticsDrawerProps) {
    const contentContainerRef = useRef<HTMLElement | null>(null);

    const [post, setPost] = useState<{ id: string; title: string; content: string; styles?: PostStyles; template_data?: PostTemplateData | null; created_at: string; quiz_id: string | null; rating_enabled?: boolean; cta_enabled?: boolean; component_order?: string[] } | null>(null);
    ...
    const { processedHtml, extractedVideos } = useMemo(() => {
        ...
        videoTags.forEach((video, index) => {
            ...
            videos.push({ src, id: null, primaryColor: null, index: iframes.length + index, placeholderId });
        });
    ...
    useEffect(() => {
        if (!isOpen || !contentContainerRef.current || clickData.length === 0) return;
        ...
        clickData.forEach((click) => {
            if (click.content_x !== null && click.content_y !== null) {
                ...
                marker.className = "absolute w-3 h-3 bg-red-500 rounded-full border-2 border-white shadow-lg pointer-events-none z-50";
    ...
    const totalRageClicks = rageClickData.reduce((sum, item) => sum + (item.totalClicks || item.click_count), 0);

    const safeTemplate = post ? normalizeTemplateData(template || post.template_data, post.created_at) : null;
        ...
        <>
            ...
            >
                <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">{post?.title || "Analytics"}</h2>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                                ...
                                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <h3 className="text-sm font-medium text-gray-600">Dead Clicks</h3>
                                    </div>
                                ...
                                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <h3 className="text-sm font-medium text-gray-600">Rage Clicks</h3>
                                    </div>
                                    <p className="text-3xl font-bold text-gray-900">{totalRageClicks}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
                                <div className="flex items-center gap-3 mb-6">
                                    <BarChart3 className="text-blue-600" size={24} />
                                    <h2 className="text-xl font-bold text-gray-900">Scroll Depth Heatmap</h2>
                                </div>
                                {scrollData.length > 0 ? (
                                            ...
                                            return (
                                                <div key={item.scroll_bucket} className="space-y-2">
                                                    <div className="flex justify-between items-center">
                                                        <span className="text-sm font-medium text-gray-700">{item.scroll_bucket}%</span>
                                    ...
                                    <p className="text-gray-500 text-center py-8">No scroll data available yet</p>
                                )}
                            </div>
                                ...
                                {clickData.length > 0 ? (
                                    <div className="space-y-4">
                                        <p className="text-sm text-gray-600">
                                            {clickData.length} total clicks ({deadClicks} dead clicks) - Click positions shown on actual post content
                                        </p>
                                                ...
                                                >
                                                    ...
                                                    <header className="mb-8">
                                                        ...
                                                        {(safeTemplate.subtitle || "").trim() ? (
                                                            <p
                                                                className={`mb-6 italic ${safeTemplate.alignment === 'center' ? 'text-center' : safeTemplate.alignment === 'right' ? 'text-right' : 'text-left'}`}
                                                                style={{
                                                                    fontFamily: safeTemplate.subtitleFont ? fontOptions.find(f => f.name === safeTemplate.subtitleFont)?.value || safeTemplate.subtitleFont : bodyFont,
                                                                    fontWeight: safeTemplate.subtitleWeight || styles.bodyWeight,
                                                        ...
                                                        <div
                                                            className={`text-sm ${safeTemplate.alignment === 'center' ? 'text-center' : safeTemplate.alignment === 'right' ? 'text-right' : 'text-left'}`}
                                                            style={{
                                                                fontFamily: safeTemplate.bylineFont ? fontOptions.find(f => f.name === safeTemplate.bylineFont)?.value || safeTemplate.bylineFont : bodyFont,
                                                                fontWeight: safeTemplate.bylineWeight || styles.bodyWeight,
                                                        ...
                                                        >
                                                            {safeTemplate.authorName ? `By ${safeTemplate.authorName}` : "By Unknown"}
                                                            {safeTemplate.date ? ` • ${new Date(safeTemplate.date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}` : ""}
                                                        </div>
                                                    </header>
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h2 className="text-xl font-bold text-gray-900">Attention by Section</h2>
                                </div>
                                {attentionData.length > 0 ? (
                                    <div className="space-y-4">
                                        {attentionData.map((item) => (
                                            <div key={item.section_id} className="space-y-2">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-sm font-medium text-gray-700">{item.section_id}</span>
                                                    <span className="text-sm text-gray-600">{Math.round(item.avg_time_visible_ms / 1000)} seconds (avg)</span>
                                                </div>
                                                <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                                                    <div
                                                        className="bg-purple-600 h-full rounded-full transition-all duration-300"
                                                        style={{ width: `${Math.min((item.avg_time_visible_ms / 60000) * 100, 100)}%` }}
                                                    />
                                                </div>
                                            </div>
                                        ))}
                                    ...
                                    <p className="text-gray-500 text-center py-8">No attention data available yet</p>
                                )}
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-6">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h2 className="text-xl font-bold text-gray-900">CTA Interaction</h2>
                                </div>
                                {ctaData.length > 0 ? (
                                    <div className="space-y-4">
                                        {ctaData.map((item) => (
                                            <div key={item.cta_id} className="p-4 bg-gray-50 rounded-lg">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="text-sm font-medium text-gray-700">{item.cta_id}</span>
                                                    <span className="text-sm text-gray-600">{item.clicked_count} clicks / {item.seen_count} seen ({item.conversion_rate.toFixed(1)}%)</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-gray-500 text-center py-8">No CTA interaction data available yet</p>
                                )}
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                                <div className="flex items-center gap-3 mb-6">
                                    <AlertTriangle className="text-red-600" size={24} />
                                    <h2 className="text-xl font-bold text-gray-900">Rage Click Heatmap</h2>
                                </div>
                                {rageClickData.length > 0 ? (
                                    <div className="space-y-4">
                                        {rageClickData.map((item, index) => (
                                            <div key={`rage-${index}-${item.x_percent}-${item.y_percent}`} className="p-4 bg-red-50 rounded-lg border border-red-200">
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="text-sm font-medium text-gray-700">Position: {item.x_percent}%, {item.y_percent}%</span>
                                                    <span className="text-sm font-semibold text-red-600">{item.totalClicks || item.click_count} clicks</span>
                                                </div>
                                                <p className="text-xs text-gray-600">Multiple rapid clicks detected at this position</p>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-gray-500 text-center py-8">No rage click data available yet</p>
                                )}
                            </div>
                        </>
                    )}
