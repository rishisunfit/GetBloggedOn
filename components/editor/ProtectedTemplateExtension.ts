            ...
            new Plugin({
                ...
                filterTransaction(tr: Transaction, state: EditorState) {
                    ...
                    for (const requiredType of REQUIRED) {
                        if (beforeTypes.has(requiredType) && !afterTypes.has(requiredType)) {
                            console.log(`[ProtectedTemplate] Blocking transaction: ${requiredType} would be removed`);
                            return false;
                        }
                    }
                    ...
                    if (hadHeader && !hasLockedTemplateHeader(tr.doc)) {
                        console.log("[ProtectedTemplate] Blocking transaction: header structure would be broken");
                        return false;
                    }
                        ...
                        if (stepAny.from !== undefined && stepAny.to !== undefined) {
                            ...
                            for (const { pos, node } of beforeNodes) {
                                ...
                                if (stepAny.from <= nodeStart && stepAny.to >= nodeEnd) {
                                    console.log(`[ProtectedTemplate] Blocking transaction: step would delete ${node.type.name} at ${pos}`);
                                    return false;
                                }
                            }
                        }
                ...
                appendTransaction(transactions, oldState, newState) {
                    ...
                    if (currentFirstChild && REQUIRED.includes(currentFirstChild.type.name as any)) {
