...
declare module "@tiptap/core" {
    interface Commands<ReturnType> {
        templateSeries: {
            setTemplateSeries: (attributes: { seriesName: string; volume: string }) => ReturnType;
            updateTemplateSeries: (attributes: { seriesName?: string; volume?: string }) => ReturnType;
        };
    }
}
...
export const TemplateSeriesExtension = Node.create<TemplateSeriesOptions>({
        ...
        return [
            {
                ...
                getAttrs: (node) => {
                    ...
                    let seriesName = element.getAttribute("data-series-name");
                    let volume = element.getAttribute("data-volume");
    ...
    addNodeView() {
        return ({ node, HTMLAttributes, getPos, editor }) => {
            ...
            seriesInput.className = "template-series-name bg-transparent inline-block focus:outline-none focus:ring-1 focus:ring-blue-500 rounded px-1";
            ...
            const updateNode = () => {
                const newSeriesName = seriesInput.value.trim() || "The Editorial Review";
                ...
                const newVolume = volumeMatch ? volumeMatch[1].trim() : volumeText.replace(/^Volume\s*/i, "").trim() || "XXIII";

                // Only update if values actually changed
                if (newSeriesName === node.attrs.seriesName && newVolume === node.attrs.volume) {
                    return;
                }
            ...
            return {
                ...
                update: (updatedNode) => {
                    ...
                    if (document.activeElement !== seriesInput && document.activeElement !== volumeInput) {
                        if (updatedNode.attrs.seriesName !== seriesInput.value) {
                            seriesInput.value = updatedNode.attrs.seriesName || "The Editorial Review";
                            resizeInput(seriesInput);
                        }
                        if (updatedNode.attrs.volume !== volume) {
                            volumeInput.value = `Volume ${updatedNode.attrs.volume || "XXIII"}`;
                            resizeInput(volumeInput);
                        }
                    }
