...
declare module "@tiptap/core" {
    interface Commands<ReturnType> {
        templateByline: {
            setTemplateByline: (attributes: { authorName: string; date: string }) => ReturnType;
            updateTemplateByline: (attributes: { authorName?: string; date?: string }) => ReturnType;
        };
    }
}
...
export const TemplateBylineExtension = Node.create<TemplateBylineOptions>({
    ...
    addAttributes() {
        return {
            authorName: {
                default: "Author Name",
                parseHTML: (element) => element.getAttribute("data-author-name") || "Author Name",
        ...
        return [
            {
                ...
                getAttrs: (node) => {
                    ...
                    let authorName = element.getAttribute("data-author-name");
    ...
    renderHTML({ node, HTMLAttributes }) {
        ...
        return [
            ...
            `By ${authorName || "Author Name"} • ${date || new Date().toLocaleDateString("en-US", {
                year: "numeric",
                month: "long",
                day: "numeric",
            })}`,
    ...
    addNodeView() {
        return ({ node, HTMLAttributes, getPos, editor }) => {
            ...
            dateInput.type = "text";
            dateInput.value = date || new Date().toLocaleDateString("en-US", {
                year: "numeric",
                month: "long",
                day: "numeric",
            });
            dateInput.className = "template-byline-date bg-transparent inline-block focus:outline-none focus:ring-1 focus:ring-blue-500 rounded px-1";
                ...
                update: (updatedNode) => {
                    ...
                    if (document.activeElement !== authorInput && document.activeElement !== dateInput) {
                        ...
                        if (updatedNode.attrs.date !== date) {
                            dateInput.value = updatedNode.attrs.date || new Date().toLocaleDateString("en-US", {
                                year: "numeric",
                                month: "long",
                                day: "numeric",
                            });
